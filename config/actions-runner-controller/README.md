...
ARC runs ephemeral, repository-scoped GitHub Actions runners for `alpar-t/baloo_export`.
...
- Scale: zero idle runners, at most one active runner.
...
The `arc-runners/github-arc-token` Secret is created manually and is never committed. It contains the `github_token` key with a fine-grained PAT selected only for repositories that use ARC. Repository-level runner registration requires repository Administration read/write and Metadata read.
