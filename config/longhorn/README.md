...
- `concurrentReplicaRebuildPerNodeLimit: 1` - Serialize replica rebuilds on each node; the live Setting is also managed in `manifests/rebuild-concurrency.yaml`
...
|-----|----------|-----------|------------|
| `weekly-backup` | Tuesday 3:00 AM | 4 weeks | All replicated volumes (single-replica `*-noreplica` SCs are excluded) |
