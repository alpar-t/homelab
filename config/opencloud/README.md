...
## Global service account (OpenCloud 7.x)

Every OpenCloud service authenticates inter-service calls with a single shared
"service account" (an OpenCloud-internal machine identity — **not** an LDAP/IDM
user; the secret is held in memory only, never persisted). It is configured
globally via the `OC_SERVICE_ACCOUNT_ID` / `OC_SERVICE_ACCOUNT_SECRET` envvars,
which every service (graph, sharing, auth-service, ...) reads.

OpenCloud **7.0 made this mandatory for the `sharing` service**. A config
generated by a pre-7.0 `opencloud init` has per-service accounts for graph /
proxy / frontend but **none for sharing**, so on upgrade the pod crash-loops
with *"The service account id has not been configured for sharing."*

Rather than hand-patch the persisted per-service config on the PVC, we supply
one account globally:

- The `secrets-init` PreSync Job (`manifests/secrets-job.yaml`) generates an
  `opencloud-service-account` Secret (fresh UUIDv4 id + random secret) if it
  doesn't already exist.
- The opencloud container consumes it via `envFrom` (`apps/opencloud.yaml`).
  Because envvars take precedence over the config file, this overrides the stale
  per-service values consistently across all services.

Since the account is in-memory only and any UUIDv4 works, generating a fresh one
is safe and reproducible on a clean install too (a fresh 7.x `init` would create
its own, which the env simply overrides).
